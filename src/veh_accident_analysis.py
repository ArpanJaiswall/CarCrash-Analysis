from pyspark.sql.functions import col, row_number, upper, sum
from pyspark.sql import Window
from src.utilities.utils import load_csv_data_to_df, write_output


class VehAccidentAnalysis:
    ...
    def count_male_accidents(self, output_path, output_format):
        ...
        df = self.df_primary_person.filter(
            (self.df_primary_person.PRSN_GNDR_ID == "MALE") & (self.df_primary_person.DEATH_CNT > 2)
        )
    ...
    def count_2_wheeler_accidents(self, output_path, output_format):
        """
        Find crashes where the vehicle body type was 2-wheelers.
        ...
        """
        df = self.df_units.filter(upper(self.df_units.VEH_BODY_STYL_ID).contains("MOTORCYCLE"))
        write_output(df, output_path, output_format)
    ...
    def top_5_vehicle_makes_for_fatal_crashes_without_airbags(
        ...
        """
        Determine the top 5 Vehicle Makers of the cars involved in crashes where the driver died and airbags did not
        deploy.
    ...
    def get_state_with_no_female_accident(self, output_path, output_format):
        """
        Determine state having highest number of accidents in which females are not involved.
        Parameters:
        - output_format (str): The file format for writing the output.
        - output_path (str): The file path for the output file.
        Returns:
    ...
    def get_top_vehicle_contributing_to_injuries(self, output_path, output_format):
        """
        Find the VEH_MAKE_IDs ranking from the 3rd to the 5th positions that contribute to the largest number of
        ...
        df = (
            ...
            .withColumn("INJR_DEATH_CNT", self.df_units["DEATH_CNT"] + self.df_units["TOT_INJRY_CNT"])
            .groupby("VEH_MAKE_ID")
            .agg(sum(col('INJR_DEATH_CNT')).alias('TOT_INJ_DEATH_CNT'))
        )

        df_top_3_to_5 = (
            df.withColumn('rn',row_number().over(Window.orderBy(col('TOT_INJ_DEATH_CNT').desc())))
            .filter("rn >=3 and rn <=5")
        ...
        return [row[0] for row in df_top_3_to_5.collect()]
    ...
    def get_crash_ids_with_no_damage_with_insuarance(self, output_path, output_format):
        """
